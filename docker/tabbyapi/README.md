...
| pi session start, ~5.6k-token prompt (`tools/session_start.py`, uncapped clocks) | turn 1 ~4.1 s cold; a turn 2 that changes turn 1's user message ~5.2 s (2.6 s with `EXL3_CONV_CKPT=2`); turn 1 of a new session in another project ~3.8 s (1.3 s from the third such session with `EXL3_CONV_CKPT=2`); in the same project ~0.5 s |
