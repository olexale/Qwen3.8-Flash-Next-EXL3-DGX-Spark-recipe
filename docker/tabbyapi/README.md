...
| `EXL3_LASTPAGE` | `1` | a prompt of exactly k×256+1 tokens keeps its last page cached (`patch_exllamav3_lastpage.py`). With the MTP drafter the fork left that page with a placeholder hash, so its K/V and the checkpoint stored after it were never found again and a follow-up prefilled from 0 (engine, 5,121-token prompt: follow-up 3.6 → 0.64 s). Bookkeeping only; the request itself computes the same. `2` also lets the exact same prompt sent again resume there (3.3 → 0.05 s), not enabled: see `PLAN_C_SESSION_START.md`. `0` = the fork's behaviour |
...
| `EXL3_DECODE_STATS` | `1` | logs one `[decode-stats]` line per finished request (`patch_exllamav3_decode_stats.py`, log-only): tokens and decode time of the thinking phase (up to `</think>`), the answer text and the tool calls (from the first `<tool_call>`), with verify rounds, drafted and accepted tokens and prompt-lookup rounds per phase, the prompt length and TTFT; counts and times only, no text. `tools/decode_report.py` turns the lines into per-phase tok/s (median, IQR, pooled) and, during an A/B trial (`arm A`/`B`), the difference with a bootstrap CI. `0` turns it off |
