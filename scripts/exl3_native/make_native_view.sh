#!/usr/bin/env bash
# A pack directory prepared for vLLM (scripts/prepare_pack.sh) carries the plugin's
...
# The third argument is for the 4.05 revision: scripts/rename_unsharded_ngram.py left the
...
[[ -f "$SRC/config.json.native" ]] || { echo "no config.json.native in $SRC (run prepare_pack.sh first)"; exit 1; }
