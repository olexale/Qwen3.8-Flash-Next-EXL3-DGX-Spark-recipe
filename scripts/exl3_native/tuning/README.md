# Native exllamav3 on GB10: launcher, benchmarks, harnesses, patches
...
| `../../beta_testing/serve_openai.sh` | (Beta) Same GB10 env and flags, OpenAI `/v1` on port 8899 (`HOST`/`PORT` override). Qwen XML → `tool_calls`. Stops on `<|im_start|>` as well as `<|im_end|>`. One Generator job at a time. Moved to beta_testing; TabbyAPI is the recommended API layer. |
